...
CPP_BASE_URL="http://localhost:6000"
...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$CPP_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$CPP_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
            --data-urlencode 'name=Yuda')
    elif [[ "$method" == "PUT" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$CPP_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
            --data-urlencode 'id=1' --data-urlencode 'name=Fajar')
    elif [[ "$method" == "DELETE" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$CPP_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
            --data-urlencode 'id=1')
    fi
