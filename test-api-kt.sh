...
KT_BASE_URL="http://localhost:6009"
...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$KT_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        if [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$KT_BASE_URL$endpoint" \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$KT_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' "${curl_data[@]}")
    elif [[ "$method" == "DELETE" ]]; then
        if [[ -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$KT_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded')
        else
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$KT_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$KT_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
