...
JS_BASE_URL="http://localhost:6008"
...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$JS_BASE_URL$endpoint")
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$JS_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
