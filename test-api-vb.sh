...
VB_BASE_URL="http://localhost:6013"
...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$VB_BASE_URL$endpoint")
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$VB_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
