...
TS_BASE_URL="http://localhost:6014"
...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$TS_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        if [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$TS_BASE_URL$endpoint" \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$TS_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' "${curl_data[@]}")
    elif [[ "$method" == "DELETE" ]]; then
        if [[ -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$TS_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded')
        else
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$TS_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$TS_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
