...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$FS_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        if [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$FS_BASE_URL$endpoint" \
        ...
        if [[ -z "$name" && -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$FS_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
                --header 'Content-Length: 0')
        elif [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$FS_BASE_URL$endpoint" \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$FS_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
    ...
    decoded_response_body=$(echo "$response_body" | sed 's/\\u0027/'\''/g')

    clean_decoded_body=$(echo "$decoded_response_body" | tr -d '[:space:]')
    ...
    if [[ -n "$expected_body" ]]; then
        if [[ "$clean_decoded_body" == "$clean_expected_body" ]]; then
            ...
            echo "Actual Response Body: $clean_decoded_body"
        fi
    else
