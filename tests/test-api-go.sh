...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$GO_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        if [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$GO_BASE_URL$endpoint" \
        ...
        if [[ -z "$name" && -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$GO_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
                --header 'Content-Length: 0')
        elif [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$GO_BASE_URL$endpoint" \
        ...
        if [[ -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$GO_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded')
        else
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$GO_BASE_URL$endpoint?id=$id" \
                --header 'Content-Type: application/x-www-form-urlencoded')
        fi
    elif [[ "$method" == "PATCH" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$GO_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
