...
test_endpoint() {
    ...
    if [[ "$method" == "GET" ]]; then
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request GET "$PY_BASE_URL$endpoint")
    elif [[ "$method" == "POST" ]]; then
        if [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request POST "$PY_BASE_URL$endpoint" \
        ...
        if [[ -z "$name" && -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$PY_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
                --header 'Content-Length: 0')
        elif [[ -z "$name" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PUT "$PY_BASE_URL$endpoint" \
        ...
        if [[ -z "$id" ]]; then
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$PY_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded')
        else
            response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request DELETE "$PY_BASE_URL$endpoint" \
                --header 'Content-Type: application/x-www-form-urlencoded' \
        ...
        response=$(curl -s -w "HTTPSTATUS:%{http_code}" --request PATCH "$PY_BASE_URL$endpoint" \
            --header 'Content-Type: application/x-www-form-urlencoded' \
